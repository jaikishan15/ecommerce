...
const {OrderItems, OrderItem} = require('../model/order-items')
...
router.delete('/:id',async(req,res)=>{
    Order.findByIdAndRemove(req.params.id).then(async order=>{
        if(order){
            ...
            return res.status(400).json({success:true,message:'the order is  not deleted'})
        }
    }).catch(err=>{
...
router.get(`/get/count`,async(req,res)=>{
    ...
    if(!orderCount){
    res.status(500).json({success:falsw});
        }
        res.send({
